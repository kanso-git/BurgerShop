...
const loadInitialIngredients = () =>
    async (dispatch, getState) => {
        try {
            ...
            const initialIngredientsAction = loadInitialIngredientsAction(ingredients.data);
            dispatch(initialIngredientsAction);
        } catch (e) {
            // error case : needs to shutdown the spinner
            dispatch(errorLoadingInitialIngredientsAction('error loading the ingredients'));
        }
    };
